...
class TeamCaptureFlagClass(GameClass):
    ...
    def __init__(self, Agents):
        ...
        self.ObservableUnits = ( SoldierClass, TankClass, TruckClass, AirplaneClass, FlagClass, WallClass  )
        ...
        self.InfiniteLifeUnits = ( WallClass, )
        self.Changes = []
...
#                PreviousNewUnit.Position = tuple(PreviousPosition)
        ...
        for UnitID1, Unit1 in State.Units.items():
            if isinstance(Unit1,FlagClass) == False and any(isinstance(Unit1,T) for T in self.InfiniteLifeUnits)==False:
                for UnitID2, Unit2 in State.Units.items():
                    if isinstance(Unit2,FlagClass) == False and any(isinstance(Unit2,T) for T in self.InfiniteLifeUnits)==False:
                        ...
                        if Unit1 != Unit2 and (Unit1.overlaps(PreviousUnit2) and Unit2.overlaps(PreviousUnit1)):
                            # Check if both Units are ByPassUnits, i.e., they cannot collide
                    ...
                    if isinstance(Unit2,FlagClass) == False and any(isinstance(Unit2,T) for T in self.InfiniteLifeUnits) == False:
                        if Unit1 != Unit2 and Unit1.overlaps(Unit2):
                                if Unit1.Attack == Unit2.Attack: # Both units are either attacking or not
                                    ...
                                    #    print(msg)
                                    RemoveUnits.add(UnitID1)
                    elif Unit1.overlaps(Unit2) and any(isinstance(Unit2,T) for T in self.InfiniteLifeUnits):
                        RemoveUnits.add(UnitID1) 
    ...
    def continueGame(self,State: TeamStateClass):
        ...
        for Unit in State.Units.values():
            UnitOwner = Unit.Owner
            UnitTeam = self.Agents[UnitOwner].TeamID
            if (UnitTeam not in TeamsLeft) and any([isinstance(Unit,T) for T in self.FlagCapturers]):
