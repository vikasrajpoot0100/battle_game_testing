...
from src.AgentTypes.RLAgent_One import Qlearning_1 
from src.AgentTypes.RLAgent_Two import Qlearning_2 
...
def getUnitAction(UnitType, ActionOptions, msg, ActionNames):
    ...
    board_func.create_output_text("Let's Start")
    # board_func.on_button_click_text()
    # board_func.create_text(f"Which of the following actions would you like to take for Unit {UnitType}") 
    
    
    # If multiple actions are available, prompt the userxt
    data_text = str(UnitType).split(".")
    board_func.create_output_text(data_text[-1])
